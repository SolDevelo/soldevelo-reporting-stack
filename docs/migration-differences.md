...
| Avg. Programmatic Stockout Rate - Last 12 months (pivot_table) | `malawi_programmatic_stockout_pivot.yaml` (pivot_table_v2) | 📝 Modernization | Same shape: cols = program_name, metric = stockout_rate. Modernized to `pivot_table_v2`. Preserves legacy `amc != 0` filter (renamed to `average_consumption`); on `mart_malawi_stock_status` (Malawi tracer products only). Row dimension switched from `processing_period_startdate` (date) to `period_name` (string label) for readability — sort order is now alphabetical on the period label, which matches date order for "MM/YYYY"-style names but could diverge under different naming conventions |
